...
const SYSTEM_UPDATES = [
  {
    ...
    title: '2026/08/12 更新',
    features: [
      '新增睡眠記錄：記錄每天就寢與起床時間（一天可多段、含午睡），在健康頁看睡眠時長趨勢；可在設定開啟隔天早上自動提醒填寫（預設關閉）',
      '新增腰圍記錄：追蹤腰圍變化，趨勢圖與體重、體脂並列',
      '新增「階段目標」：為體重、體脂、肌肉、腰圍或 BMI 設定目標值與目標日期，自動追蹤進度、達標自動打勾並收進「過往目標記錄」；也可設自訂目標手動打勾。可在設定隱藏整個功能',
      '健康頁改版（原體組成頁）：今日體重、睡眠、腰圍集中在最上方一次填完，下方依序是各項趨勢圖與完整體組成記錄，一頁看完',
      '飲食日誌新增搜尋：右上角放大鏡可查你吃過的食物與當時熱量',
      '手動輸入的餐點可一鍵加入食物庫：編輯時按「＋食物庫」會自動帶入名稱與營養，下次直接選用',
      '新增「一週開始」設定：可選週一或週日為一週起點，飲食月曆、週報區間、本週統計都會跟著調整',
      '週報匯出新增睡眠與腰圍資料',
      '新增多種常用食材：松坂豬、梅花豬、牛梅花、虱目魚肚、芋頭、干貝、熟鮭魚等（生／熟，附每 100 克數值）',
    ],
    fixes: [
      '多裝置更穩定：今天已記錄過的體重或睡眠，換另一台裝置開啟時不會再重複跳出提醒',
      '體重每日提醒改為凌晨 4 點後才詢問，剛過午夜不打擾',
      '睡眠時間改為點選式輸入（仿手機鬧鐘，一次選時與分）',
    ],
  },
  {
    version: '2026-07-18',
    title: '2026/07/18 更新',
    features: [
      '新增雲端同步：登入 Google 帳號後，飲食日誌、食材庫、習慣、體組成等資料會自動備份到雲端，每 5 分鐘同步一次，換手機或電腦都能接續使用',
      '新增電腦版版面：用電腦（寬螢幕）開啟時，改用專為大畫面設計的版面——左側選單、儀表板首頁、食譜雙欄、食材庫表格、日誌工作台等，不再只是把手機畫面放大。手機操作維持原樣不變',
    ],
    fixes: [],
  },
  {
